...
def cadastrar_comunicado(titulo, descricao, idanexo):
    try:
        conexao = ConexaoBanco()
        conexao.cursor.execute('''
            INSERT INTO comunicados (titulo, descricao, idanexo)
            VALUES (?, ?, ?)
        ''', (titulo, descricao, idanexo,))
...
def cadastrar_novo_comunicado(modelRequest):
    ...
    for idanexo in json.loads(anexos_cadastrados)["ids_inseridos"]:
        resultado = cadastrar_comunicado(titulo, descricao, idanexo)
